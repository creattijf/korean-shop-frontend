...
let allProducts = [];
    ...
    async function initApp() {
        try {
            ...
            if (!productsResponse.ok) throw new Error(`Ошибка загрузки товаров: ${productsResponse.status}`);
    ...
    function displayCategoryFilters(categories) { if (!filtersContainer) return; filtersContainer.innerHTML = '<button class="filter-btn active" data-filter="all">Все</button>'; categories.forEach(category => { const button = document.createElement('button'); button.className = 'filter-btn'; button.dataset.filter = category.slug; button.textContent = category.name; filtersContainer.appendChild(button); }); addFilterButtonListeners(); }
    ...
    function addAllEventListeners() { document.querySelectorAll('.btn-details, .product-card__image-wrapper').forEach(el => { el.addEventListener('click', (e) => { const productId = parseInt(e.currentTarget.dataset.id, 10); const product = allProducts.find(p => p.id === productId); if (product) showProductInModal(product); }); }); document.querySelectorAll('.favorite-icon').forEach(icon => { icon.addEventListener('click', (e) => { const productId = parseInt(e.currentTarget.dataset.id, 10); toggleFavorite(productId); }); }); document.querySelectorAll('.look-product-card').forEach(card => { card.addEventListener('click', (e) => { const productId = parseInt(e.currentTarget.dataset.id, 10); const product = allProducts.find(p => p.id === productId); if (product) showProductInModal(product); }); }); }
